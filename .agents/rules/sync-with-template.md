...
**BLOCKING: treat every repo as a generated one, including the template repo itself.** A synced file's local edits are reverted by the next sync unless the same change is also made upstream. Before committing any change, read `.github/sync-with-template.yml` to check whether the changed file is covered by `includes`/`excludes`, then run the template-source check below to confirm whether the file exists upstream. Run this check even when the edit looks small, mechanical, or purely cosmetic (reordering keys, a one-line tweak): a sync reverts a trivial edit exactly like a large one. When you change a synced file, make the change in the current project AND its template(s), or use an escape hatch:
